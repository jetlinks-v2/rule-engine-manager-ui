...
计划产物绑定只使用 `canvas-changes`，`steps`/`completion` 须为结构化对象而非 JSON 字符串。
...
当前戳为 `2026091711`。父页 `orderRuleEditorRemoteTools` 以 `agentVisible !== false` 再过滤一次模型目录；隐藏工具仍可通过 iframe `executeTool` 内部调用。
