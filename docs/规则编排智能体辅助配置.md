...
当前页默认写入路径是 search + node-type detail 后直接调用 `rule_editor_apply_canvas_actions`，不以可点击应用按钮或 `#ruleEditorProposal=` 链接作为成功路径。
...
`connect_nodes_batch`、`layout_nodes`、`focus_node`、`get_tool_manual`、`list_node_templates`、`get_debug_logs`、`propose_canvas_actions` 对模型 `agentVisible: false`。
`search_node_types` 使用空白/标点切词与 CJK 重叠二元组，任意 token 命中已有文案即可，空 keyword 按 `priority` TopN；不维护业务同义词表。
apply 描述首句钉死全名 `rule_editor_apply_canvas_actions`，禁止发明同前缀别名。
父页 `RULE_EDITOR_RESOURCE_VERSION` 必须与 iframe 脚本 `?v=` 保持同一戳，否则浏览器会继续缓存旧 `index.html`。
当前戳为 `2026091717`。父页 `orderRuleEditorRemoteTools` 以 `agentVisible !== false` 再过滤一次模型目录；隐藏工具仍可通过 iframe `executeTool` 内部调用。
