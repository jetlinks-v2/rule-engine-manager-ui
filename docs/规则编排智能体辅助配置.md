...
`rule_editor_apply_canvas_actions` 的拓扑图是 `client-presentation`，计划只走闭合业务 schema。
iframe executor 只返回 verified `topology` 快照，不发 `presentation`。父页 adapter 在 `complete-topology` 已满足、快照未截断且至少 2 节点 1 连线、且 iframe 未带 presentation 时，用同一份 topology 合成 `presentation.mermaid`，再发出 `topology-diagram` binding（`text/vnd.mermaid` + `presentation.flowchart`）。preferred `flowchart` 的 `preferredInputShapes` 必须是 `presentation.*`，这样 `consumerPorts()` 才会发出 PRESENTATION 端口，终答才能像 ECharts 一样自动挂卡；`diagram.flowchart` 会被当成 STRUCTURED_DATA 编译输入。已有 mermaid 仍按精确匹配校验，错误图不覆盖。`partial-draft`、失败、回滚、单节点或截断快照不合成。规则编排模块注册 preferred `flowchart` 展示能力（`deliveryPolicy=preferred`），不改全局 mermaid `explicit`。模型可见的 apply 成功结果是精简证据信封：`changes` + `completion` + `canvasRevision` + 合成的 `presentation.mermaid`；**不包含** `topology` 节点标签/连线（capture 会 redact presentation，模型不能再从 TOOL 结果抄 `graph LR`）。证据 facts 只保留 `topologyNodeCount` / `topologyLinkCount` / `topologyDiagramAvailable`。iframe RPC 仍带 topology 供父页合成 mermaid，不改 executor 原始结果。`results` 与 `configApplications`/`appliedConfig` 只留在 executor 内部。`get_node_type_detail` 截断模板时优先非 flatten、含 `from "` / topic / `dual` 的条目，并给出 `sqlPreview`；手册把 `from-sources` / `topic-reference` / `syntax-functions` / `quick-start` / `common-patterns` 排在 `rule-flow-*` 前面。模型正文不得再写 Mermaid / ASCII / 拓扑清单 / JSON AnswerSpec / 未验证资源 URI（含 `scheme://`），也不得把 `appliedConfig` 写入用户可见正文；图由客户端从 binding 自动组合。
FLAT 下该工具必须保持模型可见声明（`exposure=auto`），并排在规则编辑器 iframe clientTools 最前；
计划产物绑定只使用 `canvas-changes`（不要发明 `canvas-actions-result`）；拓扑卡片来自 verified `topology-diagram` 绑定，不是第二个写计划产物。`steps`/`completion` 优先提交结构化数组/对象；发布 apply/propose `_schema` 以 `oneOf` 同时接纳该值的 JSON 字符串（completion 还可为 mode 字符串）。父页 `coerceApplyCanvasPlanArguments` 先修复 JSON 字符串字面量内未转义的换行/回车/制表符，再解析后进 iframe；仍无法解析或截断的字符串保持 `invalid_arguments`，并提示下一轮提交结构化数组/对象而不是字符串包装。不得把 stringify 写成推荐路径。compile `CANVAS_PLAN_SCHEMA` 仍要求真实 array/object。
...
当前戳为 `2026091716`。父页 `orderRuleEditorRemoteTools` 以 `agentVisible !== false` 再过滤一次模型目录；隐藏工具仍可通过 iframe `executeTool` 内部调用。
...
> 当前契约以文首与第十二阶段为准：`topology-diagram` 使用 `text/vnd.mermaid` + `presentation.flowchart`，preferred `flowchart` 的 `preferredInputShapes` 必须是 `presentation.*`，全局 mermaid 仍为 `explicit`。下文保留当时用 `application/vnd.mermaid` / `diagram.flowchart` 绑定的交付记录，不再作为现行 shape。
