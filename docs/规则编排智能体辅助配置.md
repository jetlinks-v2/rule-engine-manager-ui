...
`rule_editor_propose_canvas_actions` 与 apply 共用同一套 `flowMode` / `completion` / `steps[].op` 计划；
用户点击后走 apply 执行器，禁止再登记隐藏写入工具的 `toolName`/`toolId`。
`toolName`/`toolId`/缺 `op` 的提案失败返回可重试的 kebab `op` 计划样例（`retryable=true`），要求本轮重试 propose，不得据此回复无法插入，也不得提示本轮改调 apply。
当前页实现订阅、转发、推送到接口、设备上线事件转发视为落地，读完节点详情后直接 `rule_editor_apply_canvas_actions`。
「帮我执行、执行、现在写入、配置到本页、应用到画布」只走 apply，不得用 propose 链接代替写入。
父页 `RuleEditor.agent.system.write` 与 iframe 手册对齐：如何做 / 最佳实践 / SQL / 转换格式仍默认咨询；“我想实现”本页流程不再默认咨询，禁止隐藏 `toolName`/`toolId`，禁止回复无法一键插入。
模型可见 apply `_schema` 是薄 object（`op` 枚举、无 6 路 oneOf）；propose 发布同一套薄 `steps[].op.enum`。完整 `CANVAS_PLAN_SCHEMA` 仅用于 compile。
...
insert / connect / layout 内部写工具仍对模型隐藏；propose 保持可见但只登记点击计划。
模型可见写入只留 `rule_editor_apply_canvas_actions` 与需确认卡的 `edit_node` / `delete_node` / `delete_link`；
`connect_nodes_batch`、`layout_nodes`、`focus_node`、`get_tool_manual`、`list_node_templates`、`get_debug_logs` 对模型 `agentVisible: false`。
...
当前戳为 `2026091618`。父页 `orderRuleEditorRemoteTools` 以 `agentVisible !== false` 再过滤一次模型目录；隐藏工具仍可通过 iframe `executeTool` 内部调用。
...
`rule_editor_propose_canvas_actions` 只登记短期点击动作，本身不修改画布。它用于“用户还在审阅方案、明确要求给入口、或助手不确定是否应立即写入”的场景；如果用户已经明确要求插入、创建、应用到画布或连接，应直接调用 `rule_editor_apply_canvas_actions`，不再让用户多点一次“插入”。咨询态提案必须使用与 apply 相同的 `flowMode` / `completion` / `steps[].op` 计划；链接内只携带提案 ID，真实计划保存在 iframe 内存中。父页面通过 `markdownLinkHandler` 拦截 `#ruleEditorProposal=<id>`，再经 bridge 触发 `execute-proposal`，点击后调用 apply 执行器，避免把配置或 SQL 塞进 URL。
...
- 全失败且因 `toolName`/`toolId`/缺 `flowMode`/`op` 时，顶层 `instruction` 给出 kebab `op` 计划样例，且 `retryable=true`。这表示计划形态错误，应在本轮重试 propose，不要提示本轮改调 apply。
- apply 若收到 propose 信封（JSON 字符串或数组 `actions`），在缺顶层 `flowMode`/`steps` 时取第一条 action / `action.plan`。
- 「帮我执行 / 执行 / 现在写入 / 配置到本页 / 应用到画布」只能调用 `rule_editor_apply_canvas_actions`，不得用 propose 链接代替写入。
...
- 方案咨询：如何做 / 最佳实践 / SQL / 转换格式回复简短可审查方案；只有用户要求入口、需要稍后自行应用，或意图不确定时，才提供“插入到画布”链接（提案与 apply 同构 `op` 计划），不展示工具名、提案编号、链接协议或内部参数。
- 明确落地：用户直接说“插入 / 创建 / 应用到画布 / 帮我执行 / 现在写入 / 配置到本页 / 配置到当前规则 / 连接”，或在本页描述要实现的订阅、转发、推送到第三方接口、设备上线事件转发时，新增节点和连线直接写入草稿，写入后只告知影响和保存要求。
