...
计划产物绑定只使用 `canvas-changes`（不要发明 `canvas-actions-result`）；拓扑卡片来自 verified `topology-diagram` 绑定，不是第二个写计划产物。`steps`/`completion` 优先提交结构化数组/对象；发布 apply/propose `_schema` 同时接纳该值的 JSON 字符串（completion 还可为 mode 字符串），由父页 `coerceApplyCanvasPlanArguments` 解析后再进 iframe。compile `CANVAS_PLAN_SCHEMA` 仍要求真实 array/object。不得把 stringify 写成推荐路径。
...
整段 `steps`/`completion` 若被写成 JSON 字符串，同样必须先过发布 schema 的 `oneOf`（array/object 或非空字符串），否则 `CommandJsonSchema` 会在 coerce 前以 `actualType=string` 拒绝。
...
当前戳为 `2026091713`。父页 `orderRuleEditorRemoteTools` 以 `agentVisible !== false` 再过滤一次模型目录；隐藏工具仍可通过 iframe `executeTool` 内部调用。
