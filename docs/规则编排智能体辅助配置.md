...
浏览器拥有的助手工具走 `defineClientTool`（`ai-client-tool-contract/v2`）：规则草稿
`rule_engine_create_rule_draft`、告警仪表盘 `alarm_dashboard_*`、告警分析 `alarm_*`。
iframe 只读工具由 `views/Instance/RuleEditor/toolRuntime.ts` 通过
`defineAiClientToolContract` 投影 typed port，不复制一份 iframe schema。
`rule_editor_apply_canvas_actions` 计划只走闭合业务 schema；写计划产物只有 `canvas-changes`。
默认 apply 成功**不**合成 mermaid、**不**发出 `topology-diagram`，也不再把流程图卡片当成终答义务：画布才是拓扑可视化，聊天只给一句摘要 + 未保存/未发布状态。iframe executor 仍只返回 verified `topology` 快照，`presentation === undefined`。父页模型可见信封是 `changes` + `completion` + `canvasRevision` + `instruction`，**不包含** topology 节点标签/连线、mermaid、`results`、`configApplications`/`appliedConfig`。证据 facts 可保留 `topologyNodeCount` / `topologyLinkCount`。若 iframe 意外带上 mermaid，仍按精确匹配校验，错误图 `invalid_result`，缺失 mermaid 不把已成功写入打成失败。规则编排模块仍可注册 type `flowchart`（`text/vnd.mermaid` + `presentation.flowchart`），但 `deliveryPolicy=explicit`，不改全局 mermaid `explicit`。模型正文不得写 Mermaid / ASCII / 拓扑清单 / JSON AnswerSpec / 内部文件句柄（`fs://`），也不得把 `appliedConfig` 写入用户可见正文；用户 http(s) 业务地址可以出现在摘要里。用户未要求看流程图时不要提供流程图卡片。通用终答审计不再把普通 `http(s)://` 当成未验证内部资源，也不再用一致性暂停文案替换已成功写入。
`get_node_type_detail` 截断模板时优先非 flatten、含 `from "` / topic / `dual` 的条目，并给出 `sqlPreview`；手册把 `from-sources` / `topic-reference` / `syntax-functions` / `quick-start` / `common-patterns` 排在 `rule-flow-*` 前面。搜索仍一次；详情要对 **steps 中每个类型各读一次**（源/整形/汇聚），不得全局只 detail 一次。`contracts.output` 有 `kind` 但缺 `fields` 时，按 kind 投影信封字段（`device-message` → `deviceId`/`timestamp`/`messageType`/`headers`）；HTTP 已有 `payload`/`contentType` 时不另发明字段。用户 JSON / 对外字段名是 TARGET；来源表达式必须来自本轮上游 `contracts.output.fields` 或 ReactorQL select 别名，禁止因为用户叫 `ts` 就写 `data.ts`。
FLAT 下该工具必须保持模型可见声明（`exposure=auto`），并排在规则编辑器 iframe clientTools 最前；
计划产物绑定只使用 `canvas-changes`（不要发明 `canvas-actions-result`）。`steps`/`completion` 优先提交结构化数组/对象；发布 apply/propose `_schema` 以 `oneOf` 同时接纳该值的 JSON 字符串（completion 还可为 mode 字符串）。父页 `coerceApplyCanvasPlanArguments` 先修复 JSON 字符串字面量内未转义的换行/回车/制表符，再解析后进 iframe；仍无法解析或截断的字符串保持 `invalid_arguments`，并提示下一轮提交结构化数组/对象而不是字符串包装。不得把 stringify 写成推荐路径。compile `CANVAS_PLAN_SCHEMA` 仍要求真实 array/object。
当前页默认写入路径是 `rule_editor_search_node_types` 一次后优先使用 `compositions[]`，再一次 `rule_editor_apply_canvas_actions`（`insert-composition` 或已展开 ops）。对不上配方时才对 steps 中每个类型各读一次详情。不以可点击应用按钮或 `#ruleEditorProposal=` 链接作为成功路径。
iframe 远端写工具（apply / edit / delete）必须由 adapter 发布 `expands.effect=WRITE`：catalog snapshot 会把缺少 typed effect 的非只读工具隔离出 `session.init.tools`，FLAT `business_execution` 再按精确 ID 调用就会变成 `model_unknown_tool`。
`rule_editor_propose_canvas_actions` 对模型 `agentVisible: false`；内部仍可通过 `executeTool` / `execute-proposal` 执行旧 Markdown 链接。禁止再登记隐藏写入工具的 `toolName`/`toolId`。
`toolName`/`toolId`/缺 `op` 的内部提案失败返回可重试的 kebab `op` 计划样例（`retryable=true`），不得据此回复无法插入。
当前页实现订阅、转发、推送到接口、设备上线事件转发视为落地，读完节点详情后直接 `rule_editor_apply_canvas_actions`。
完整订阅/转换/转发必须在同一次 apply 的 `steps` 写全所有 `connect`，`completion` 用 `complete-topology`；不要先插入再第二次 apply 只连线。`partial-draft` 仅在用户只要局部未完成草稿时使用。
字符串 `source`/`target`/`node` 引用会在 compile 与父页 coerce 提升为 `{kind:"alias",value}`；发布 apply/propose `_schema` 接受 string | `{kind,value}`，否则后端 `CommandJsonSchema` 会在 coerce 前按 object-only 拒绝。
整段 `steps`/`completion` 若被写成 JSON 字符串，同样必须先过发布 schema 的 `oneOf`（array/object 或非空字符串），否则 `CommandJsonSchema` 会在 coerce 前以 `actualType=string` 拒绝。pretty-print 或配置字符串里的裸换行由 coerce 只在 JSON 字面量内转义后再 `JSON.parse`；不要发明缺失括号去补全截断 JSON。
iframe `context-change` 在 digest 不变或客户端工具调用进行中时不递增 `contextVersion`，父页不因 context-only tick 刷新 session 工具契约，避免同轮第二次 apply 变成 `error.agent_client_channel_not_ready`。
「帮我执行、执行、现在写入、配置到本页、应用到画布」只走 apply。
父页 `RuleEditor.agent.system.write` 与 iframe 手册对齐：如何做 / 最佳实践 / SQL / 转换格式且无落地意图时仍咨询；禁止隐藏 `toolName`/`toolId`，禁止回复无法一键插入，禁止把可点击应用按钮当成成功路径。
模型可见 apply `_schema` 是薄 object（`op` 枚举、无 6 路 oneOf）。完整 `CANVAS_PLAN_SCHEMA` 仅用于 compile。
extract/compile 接受 `steps[].op` 的 underscore/hyphen 拼写别名和字符串 completion mode；仍拒绝 `toolName`/`toolId`。apply 会解开 JSON 字符串或数组 `actions` 信封。
insert / connect / layout / propose 内部写工具对模型隐藏。
模型可见写入只留 `rule_editor_apply_canvas_actions` 与需确认卡的 `edit_node` / `delete_node` / `delete_link`；
`connect_nodes_batch`、`layout_nodes`、`focus_node`、`get_tool_manual`、`list_node_templates`、`get_debug_logs`、`propose_canvas_actions`、`find_nodes` 对模型 `agentVisible: false`。
`search_node_types` 使用空白/标点切词与 CJK 重叠二元组，任意 token 命中已有文案即可，空 keyword 按 `priority` TopN；命中时附带最多 1～3 条 `compositions[]`。不维护业务同义词表，不新增平行 composition 搜索工具。
apply 描述首句钉死全名 `rule_editor_apply_canvas_actions`，`steps[].op` 含 `insert-composition`（compile 前展开），禁止发明同前缀别名。
父页 `RULE_EDITOR_RESOURCE_VERSION` 必须与 iframe 脚本 `?v=` 保持同一戳，否则浏览器会继续缓存旧 `index.html`。
当前戳为 `2026091719`。父页 `orderRuleEditorRemoteTools` 以 `agentVisible !== false` 再过滤一次模型目录；隐藏工具仍可通过 iframe `executeTool` 内部调用。
验证：`pnpm -C runtime-ui -F rule-engine-manager-ui test:agent-tools`，以及
`node --test modules/rule-engine-manager/src/test/js/ai-agent-canvas-plan.test.mjs`。
...
当前嵌入态通过 `hideHeader=true` 隐藏了 Node-RED 原生 Header；Node-RED 的导入能力仍已注册，
但外层头部只提供保存、发布和关闭，画布右键菜单也只提供导出，导致用户无法在当前展示方式下
导入规则 JSON。目标是在不恢复完整 Node-RED Header、不复制 Node-RED JSON 解析和冲突处理逻辑的
前提下，让用户可以在外层编辑器头部使用导入和导出。

目标：

- 在规则编辑器外层头部提供导入、导出操作，复用 Node-RED 原生导入/导出对话框。
- 导入只修改当前 iframe 内未发布画布并触发 dirty 状态；保存、发布仍沿用现有动作和校验流程。
- 导入 bridge 复用现有父子窗口 origin、ruleId 校验，不把 JSON 内容、文件内容或内部 action ID 放入 URL。
- 中英文按钮、Tooltip、状态和失败提示全部通过现有 `rule-engine-manager-ui` i18n 提供。

影响范围：

- `runtime-ui/modules/rule-engine-manager-ui/views/Instance/RuleEditor/RuleEditorHeader.vue`：增加导入、导出入口。
- `runtime-ui/modules/rule-engine-manager-ui/views/Instance/RuleEditor/index.vue`：编排头部操作与 iframe bridge 对接。
- `runtime-ui/modules/rule-engine-manager-ui/views/Instance/RuleEditor/useRuleEditorAgentBridge.ts`：增加受控的 editor action 请求和结果处理。
- `modules/rule-engine-manager/src/main/resources/static/rule-editor/ai-agent-bridge.js`：校验父页面请求后调用 Node-RED 原生导入/导出 action，并回传完成或失败状态。
- `runtime-ui/modules/rule-engine-manager-ui/locales/lang/zh.json` / `en.json`：补齐用户可见文案。

不做：

- 不移除 `hideHeader=true`，不恢复完整 Node-RED Header 或其项目、调色板等额外入口。
- 不在父页面实现 JSON 解析、节点 ID 冲突合并、节点类型校验或导出格式转换。
- 不新增后端导入导出接口，不绕过当前规则保存 / 发布 API，也不自动发布导入结果。
- 不把导入的节点类型、脚本或配置当作可信内容绕过 Node-RED 自身校验。

实施步骤：

1. 复用现有 bridge 的父子 origin、ruleId 校验，新增 `show-import-dialog` 和 `show-export-dialog` 两类受控请求；iframe 内分别调用 `RED.actions.invoke("core:show-import-dialog")` 与 `RED.actions.invoke("core:show-export-dialog")`。
2. 外层头部增加图标按钮和 Tooltip；导入完成后仅刷新 bridge context / dirty 状态，导出完成后保持 Node-RED 原生下载行为。
3. 将导入/导出动作状态纳入头部 loading、不可用和错误反馈，避免在 iframe 未 ready 或规则已关闭时继续发送请求。
4. 补齐中英文 i18n、bridge 单元 / 静态脚本校验和规则编辑器模块构建。
5. 浏览器验证 JSON 文件导入、文本导入、导出、导入后保存、导入冲突提示和关闭未保存草稿等路径。

风险与待确认点：

- Node-RED 原生导入对话框是异步 UI action，bridge 需要监听对话框确认后的画布变更，不能把“打开对话框”误报为“导入完成”。
- 导入外部 JSON 可能引用当前边缘端未安装的节点类型；这类问题继续由 Node-RED 原生校验和现有保存校验反馈。
- 导出结果仍由 iframe 触发浏览器下载，不经过父页面文件读取，避免跨 iframe 传输大 JSON。

验证方式：

- `node --check modules/rule-engine-manager/src/main/resources/static/rule-editor/ai-agent-bridge.js`。
- `pnpm -C runtime-ui build:modules rule-engine-manager-ui`、中英文 locale JSON 解析和相关目录 `git diff --check`。
- 浏览器在真实规则编辑器中验证导入、导出、保存和取消导入，确认 iframe 未 ready、父页面 origin 不匹配时请求被拒绝。
...
> 当前契约以文首与第十六阶段为准：默认 apply 成功只有 `canvas-changes`，不发 `topology-diagram`，flowchart 能力为 `explicit`。下文第九到十五阶段挂卡/合成 mermaid 的记录不再作为现行义务。
...
本阶段仍不做批量删除和自动保存 / 部署。新增节点、按模板插入节点、修改已有节点配置、连接单条或多条连线、指定节点自动排版都只是编辑器草稿；当前规则编辑器页默认由 `rule_editor_apply_canvas_actions` 一次写入并告知草稿已变更。删除节点 / 连线继续保留确认。不要把可点击应用按钮或 `#ruleEditorProposal=` 链接当成成功路径；如何做 / SQL / 最佳实践且无落地意图时才保持咨询回复。
...
`rule_editor_propose_canvas_actions` 对模型隐藏（`agentVisible: false`），只作为内部点击执行器保留。当前页成功路径是 `rule_editor_apply_canvas_actions` 直接写入草稿，不以可点击应用按钮作为助手成功 UX。旧会话里的 `#ruleEditorProposal=<id>` 仍可由父页 `markdownLinkHandler` 拦截后走 `execute-proposal` / apply 执行器。

内部提案只接受 apply 同构计划：

- `{ label, flowMode, completion, steps }`，或 `{ label, plan: { flowMode, completion, steps } }`。
- `steps[].op` 为 `insert-node` / `insert-template` / `edit-node` / `connect` / `connect-batch` / `layout`，节点引用用 alias / node-id。模型可见 propose schema 与 apply 共用该薄 `op` 枚举，不发布 6 路 compile oneOf。
- extract/compile 会把 `insert_node`、`link_nodes` 等拼写别名归一到 kebab `op`，并把字符串 `completion: "complete-topology"` 提升为 `{ mode }`；含 `toolName` / `toolId` 的步骤一律拒绝，不做隐藏写入工具 ID 翻译。别名不写入手册或文案。
- 全失败且因 `toolName`/`toolId`/缺 `flowMode`/`op` 时，顶层 `instruction` 给出 kebab `op` 计划样例，且 `retryable=true`。这只服务内部旧链接执行，不作为模型可见成功路径。
- apply 若收到 propose 信封（JSON 字符串或数组 `actions`），在缺顶层 `flowMode`/`steps` 时取第一条 action / `action.plan`。
- 「帮我执行 / 执行 / 现在写入 / 配置到本页 / 应用到画布」只能调用 `rule_editor_apply_canvas_actions`。
- 多个独立 `actions` 默认会被拒绝；只有多个动作确实是互斥备选方案时，才可显式传 `allowMultiple=true`。

模型可见 apply `_schema` 是薄 object（`op` 枚举、无 6 路 oneOf），完整 `CANVAS_PLAN_SCHEMA` 的 6 路 oneOf 仅用于 compile。

边界：

- 修改节点配置、删除节点、删除连线等需要确认的工具不会被登记为一键应用，仍走原确认卡片。
- 提案有短期 TTL 和数量上限；sessionStorage 可恢复 canvas-plan 项。刷新前登记的旧 `toolName` 缓存仍可兼容执行，新登记不再写入该形态。
- 点击应用后只是修改编辑器草稿，保存 / 发布仍由编辑器原生按钮完成。

交互分层：

- 方案咨询：如何做 / 最佳实践 / SQL / 转换格式且无落地意图时，回复简短可审查方案，不要提供可点击应用按钮。
- 明确落地：用户直接说“插入 / 创建 / 应用到画布 / 帮我执行 / 现在写入 / 配置到本页 / 配置到当前规则 / 连接”，或在本页描述要实现的订阅、转发、推送到第三方接口、设备上线事件转发时，search + detail 后直接 `rule_editor_apply_canvas_actions`，写入后只告知影响和保存要求。
- 高风险修改：修改配置、删除节点、删除连线继续保留确认，避免误改当前规则。

推荐交互策略：

- 互动对话是必要的，但只承载业务选择，例如 topic 范围、时间窗口、目标格式、是否需要下游输出；不把“查工具、读模板、准备参数”等内部过程暴露给用户。
- 当前页默认回复形态是直接写入草稿后的简短结果说明，不以一键应用链接为成功路径。
...
- done：第十六阶段：默认 apply 成功不再产生流程图卡片。去掉 `topology-diagram` producer 与 mermaid 合成；`RULE_EDITOR_FLOWCHART_PRESENTATION.deliveryPolicy=explicit`；画布是唯一拓扑可视化。搜索仍一次，`get_node_type_detail` 按计划中每个类型各读一次；`kind` 缺 fields 时投影 `device-message` 信封字段；用户 JSON 键是 TARGET，禁止 `data.<targetKey>`。资源戳 `2026091717`。质量检查补强：iframe RPC 仍可把意外 mermaid 交给父页做 mismatch `invalid_result`，父页再从模型可见信封去掉；信封填充只看 owner 声明的空 `fields`，不覆盖已声明字段。`pnpm -C runtime-ui -F rule-engine-manager-ui test:agent-tools` runtime 35、catalog 12（line 99.60%，branch 91.29%）；`ai-agent-canvas-plan.test.mjs` 70。未用真实会话复验 TERMINAL_EVIDENCE_PAUSED 是否消失；未用浏览器复验重启后 iframe 是否拉取新 HTML。
- done：第十七阶段第一刀：节点只声明契约与边界；iframe `ai-agent-composition.js` 按当前 palette 配对线性 `source → [adapter?] → sink`；kind 适配器运行时生成一层 reshape；配方挂在现有 `search_node_types.compositions[]`（最多 1～3 条）；`insert-composition` 只作为 apply op，compile 前展开；隐藏 `find_nodes`；apply 建议摘要无 URL。资源戳 `2026091718`。质量检查补强：compositions 命中 cap=3 时 `truncated=true`；桥接 apply 走 expand-before-compile，executor 看不到 `insert-composition`。`pnpm -C runtime-ui -F rule-engine-manager-ui test:agent-tools` runtime 35、catalog 13（line 99.60%，branch 91.29%）；`ai-agent-canvas-plan.test.mjs` 75。不写 `composesWith`、不上线/HTTP 同义词表、不改 compile 校验 / Agent Java。请求响应/定时骨架留待后续切片。待真实会话烟测配方一次写入；未用浏览器复验重启后 iframe 是否拉取新 HTML。
- done：第十八阶段：通用终答 URI 审计收窄为内部 scheme；剥离载体后保留业务结论，不用一致性暂停文案替换已成功写入。规则编辑器提示词允许用户 http(s)，仍禁止内部 `fs://`。资源戳 `2026091719`。聚焦 Java 回归 25 通过；`test:agent-tools` runtime 35、catalog 13；`ai-agent-canvas-plan.test.mjs` 75。未用真实会话复验 webhook 终答。
- done：第十五阶段：模型可见 apply 成功结果去掉 `topology` 节点标签/连线，避免模型从 TOOL 结果抄 `graph LR`；capture 仍从 `$.presentation.mermaid` 绑 `topology-diagram`，证据 facts 只保留节点/连线计数。iframe RPC 仍带 topology 供父页合成 mermaid，不改 executor 原始结果。`get_node_type_detail` 截断时优先非 flatten、含数据源 SQL 的模板并给出 `sqlPreview`，手册把方言/数据源条目排在 `rule-flow-*` 前面。资源戳 `2026091716`。`pnpm -C runtime-ui -F rule-engine-manager-ui test:agent-tools` runtime 35、catalog 12（line 99.60%，branch 91.52%）；`ai-agent-canvas-plan.test.mjs` 68。未用真实会话复验模型是否停止手写 Mermaid；未用浏览器复验重启后 iframe 是否拉取新 HTML。
- done：第十四阶段：模型可见 apply 成功结果投影为精简证据信封（`changes` / `completion` / `canvasRevision` / `topology` / 合成 `presentation`）。`results` 与 `configApplications`/`appliedConfig` 只留在 executor 内部，父页与 iframe 成功返回都不投影给模型，避免 URL / 源码污染 COMPOSITE 终答。资源戳 `2026091715`。`pnpm -C runtime-ui -F rule-engine-manager-ui test:agent-tools` runtime 35、catalog 12（line 99.60%，branch 91.59%）；`ai-agent-canvas-plan.test.mjs` 67。质量检查补强：模型可见信封对 `changes` / `completion` / `topology` / `validation.issues` 做字段白名单，禁止把 URL/源码挂在这些对象的额外字段上浅拷贝进模型上下文；父页合成 mermaid 后再投影，不改 iframe 原始结果。未改 CanvasPlanCompiler / 全局 mermaid / Agent Java。未用真实会话复验 COMPOSITE 终答是否因去掉 runtime dump 而不再暂停。
- done：第十三阶段：JSON 字符串 `steps`/`completion`/`actions` 在 Stage 11 `oneOf` 准入后，父页 coerce 与 iframe `parseJsonStructured` 修复字符串字面量内未转义的 `\n`/`\r`/`\t` 再解析。不发明截断 JSON 的括号。剩余非法字符串 `invalid_arguments`，提示下一轮提交结构化数组/对象而不是字符串包装。资源戳 `2026091714`。`pnpm -C runtime-ui -F rule-engine-manager-ui test:agent-tools` runtime 34、catalog 12；`ai-agent-canvas-plan.test.mjs` 66。质量检查补强：pretty-print 结构空白、已转义 `\\n` 不双转义、字面量内截断 fail-closed；收回模型可见 schema 对 pretty-print repair 的推荐表述。未把 stringify 写成推荐路径；未改 presentation.flowchart / 全局 mermaid / Agent Java。未用真实会话复验带裸换行 SQL 的 apply 是否写画布。
- done：第十二阶段：把 `topology-diagram` producer shape 与 preferred `flowchart` consumer 对齐为 `presentation.flowchart`，使 `consumerPorts()` 发出 PRESENTATION 端口，终答可像 ECharts 一样自动挂卡。保留 `text/vnd.mermaid` 与 type `flowchart`，不改全局 mermaid explicit。guide 明确写计划绑定仍是 `canvas-changes`，拓扑卡片来自 `topology-diagram`。提示词禁止 `scheme://` 与把 `appliedConfig` 写入用户可见正文。资源戳 `2026091713`。`pnpm -C runtime-ui -F rule-engine-manager-ui test:agent-tools` runtime 33、catalog 12；`ai-agent-canvas-plan.test.mjs` 65。未用真实会话复验 COMPOSITE 终答是否自动挂卡。
- done：第十一阶段：发布 apply/propose `_schema` 以 `oneOf` 接纳 JSON 字符串 `steps`/`completion`（及 completion mode 字符串），复用父页 coerce；compile `CANVAS_PLAN_SCHEMA` 仍拒绝字符串。资源戳 `2026091712`。`pnpm -C runtime-ui -F rule-engine-manager-ui test:agent-tools` runtime 33、catalog 12；`ai-agent-canvas-plan.test.mjs` 65。未自动改写 `flowMode`。未用真实会话复验 `CommandJsonSchema` 对字符串 `steps` 的 admission。
- done：第十阶段：`topology-diagram` 改为 `text/vnd.mermaid`，并注册 preferred `flowchart` 展示能力（不改全局 mermaid explicit）。提示词不再要求选择 renderer。资源戳 `2026091711`。`pnpm -C runtime-ui -F rule-engine-manager-ui test:agent-tools` runtime 33、catalog 12；`ai-agent-canvas-plan.test.mjs` 64。未用真实会话复验 COMPOSITE 终答是否自动挂卡。
- done：第九阶段父页从 iframe topology-only 成功结果合成 `presentation.mermaid`，使 `topology-diagram` binding 进入终答证据。iframe executor 仍不发 presentation；错误 mermaid 不覆盖；`partial-draft` / 单节点 / 截断快照不合成。超过 16KB 的 mermaid 不合成、不把已成功 apply 打成失败。资源戳仍为 `2026091621`。`pnpm -C runtime-ui -F rule-engine-manager-ui test:agent-tools` runtime 31、catalog 12。
- done：第八阶段归一字符串节点引用、一次 apply 写全连线、context digest / in-flight 不拆客户端通道。`error.agent_client_channel_not_ready` 映射为可重试 dependency。发布 apply/propose schema 接受 string | `{kind,value}` 节点引用，避免后端 `CommandJsonSchema` 在 coerce 前拦截 `"msgSub"`。资源戳 `2026091621`。`node --test .../ai-agent-canvas-plan.test.mjs` 64；`pnpm -C runtime-ui -F rule-engine-manager-ui test:agent-tools` runtime 26、catalog 12。未用真实会话复验字符串引用过后端 admission；未用浏览器复验新 HTML 缓存戳。
- done：第七阶段默认自动写入画布。根因是 apply `write:true` 投影为 `readOnlyHint:false` 却没有 typed effect，`createAiClientToolCatalogSnapshot` 按 `effect_required_for_side_effect` 隔离出 `wireDefinitions`，FLAT `business_execution` 调用精确 ID 得到 `model_unknown_tool`。adapter 现为远端写工具发布 `expands.effect=WRITE`；propose 对模型隐藏；资源戳 `2026091619`。
...
- fixed：2026-09-16 父页 `RuleEditor.agent.system.write` 不再把“我想实现”默认成咨询；如何做 / 最佳实践 / SQL / 转换格式仍咨询。本页订阅、转发、推送到第三方接口、设备上线事件转发视为落地，读完节点详情后直接 `rule_editor_apply_canvas_actions`，提案仍用同一套 `op` 计划。
- commit：pending
- PR：pending
...
## 2026-08-11 告警分析通用智能体迁移计划

当前结论：将 `ui/modules/alarm-ui/agentCapabilities/alarmAnalysis` 的告警分析通用智能体能力迁入当前模块，由 `rule-engine-manager-ui/register.ts` 统一注册。保留既有 provider ID、extension key、工具能力 ID 和受控入口 namespace，避免影响既有能力路由与会话上下文；旧 `alarm-ui/agentCapabilities` 目录暂不删除，但取消其重复注册。

模块独立性是硬约束：`rule-engine-manager-ui` 的产物在 `alarm-ui` 整体删除后仍须独立编译和运行。目标代码不得 import `alarm-ui`、不得通过别名深度引用其私有文件、不得动态加载其模块，也不得通过 `alarm-ui/register.ts` 暴露的 `apis` 间接调用。迁移时读取旧代码只作为一次性来源，所需请求、类型、数据归一化、受控入口生产与消费逻辑均由目标模块自行持有。

影响范围与 owning module：

- `ui/modules/rule-engine-manager-ui/agentCapabilities/alarmAnalysis`：承接 provider、工具、工作流、视联场景匹配和测试，并增加只服务智能体的告警数据适配层。
- `ui/modules/rule-engine-manager-ui/register.ts`：新增 `generalAgentExtensions` 注册，模块 owner 调整为 `rule-engine-manager-ui`。
- `ui/modules/rule-engine-manager-ui/locales/lang/zh.json`、`en.json`：迁入 `AlarmGeneralAgent.*` 文案。
- `ui/modules/rule-engine-manager-ui/views/Alarm/Log`：消费一次性 `agentEntry` 上下文，通过正常告警权限查询打开目标记录；菜单锚点使用 `rule-engine/Alarm/Log`，看板锚点使用 `rule-engine/DashBoard`。
- `ui/modules/alarm-ui/register.ts`：只移除旧 `generalAgentExtensions` 注册及对应 import，保留当前 `moduleId`、`apis`、`dataCapabilityProviders` 和其他未提交改动。

明确不做：

- 不删除 `ui/modules/alarm-ui/agentCapabilities` 旧目录，不清理旧仓库 locale 和历史计划文档。
- 不迁移或修改 `alarm-ui/dataCapabilities`，不覆盖其 API、locale、register 中正在进行的数据能力改动。
- 不把约 1900 行的页面级 `views/alarmEvents/dataSource.ts` 整体复制到目标仓库；只抽取智能体实际使用的请求、类型和安全字段映射。
- 不调整告警页面壳层、菜单信息架构、后端接口或数据结构。
- 不保留 `alarm/dashboard`、`alarm/alarmEvents`、`alarm/alarmSearch` 等旧模块菜单锚点，不以旧告警事件页作为记录回跳承载。

实施步骤：

1. 复制告警智能体 provider、tools、services、视联场景匹配与测试到目标模块，保持公共标识兼容，并将 owner 与菜单锚点完整切换到 `rule-engine-manager-ui`。
2. 在目标能力目录内建立自包含的数据适配层，承接告警记录、历史、处理记录、聚合、级别、AI 场景、空间绑定和 AI 复核统计接口，避免深度依赖 `alarm-ui` 私有页面代码。
3. 在目标告警记录页接入受控入口消费逻辑，按记录 ID 重新经过正常权限查询后打开记录；失效、已删除或无权记录回退到普通列表。
4. 迁入中英文 `AlarmGeneralAgent.*` 文案并在目标 `register.ts` 注册扩展；精确移除源 `register.ts` 的旧扩展注册，防止同一 extension key 重复加载。
5. 回归 provider 能力、工具返回结构、菜单权限、记录回跳和视联场景搜索。

风险与验证：

- 风险：源页面数据层兼容了多种历史告警返回 shape；抽取适配层时必须保留智能体输出所需的 ID、来源、目标、级别、状态、时间、摘要和历史处理字段，不能直接透出原始后端对象。
- 风险：目标告警记录页当前没有 `agentEntry` 消费逻辑，必须验证设备和视联目标类型均能由正常权限接口查询；不支持的类型应安全失败，不能错误展示其他记录。
- 验证：执行 `visionSceneSearch.test.mjs`、目标模块 `pnpm run test:agent-tools`、新增适配层定向测试、locale JSON 解析与键对称检查、目标代码的 `alarm-ui` import / 动态加载 / 模块 API 零引用扫描，以及相关仓库 `git diff --check`。考虑本机资源和当前依赖状态，默认不执行全量构建；若定向验证不足，再先说明后执行模块构建。

验证结果：

- passed：`pnpm run test:alarm-agent` 通过视联场景搜索与告警数据适配层定向测试；`pnpm run test:agent-tools` 通过 12/12 适配器测试及 1/1 真实 core catalog 类型路由测试。
- passed：目标中英文 locale JSON 均可解析，`AlarmGeneralAgent.*` 各 83 个且 key 集合完全对称；迁移文案来自源模块已提交版本，未带入 `alarm-ui` 当前未提交的数据能力文案。
- passed：目标生产代码对 `alarm-ui` 的静态 import、别名深度引用、动态加载和模块 API 引用均为零；文档中仅保留迁移边界说明。扩展只在 `rule-engine-manager-ui/register.ts` 注册，旧 `alarm-ui/agentCapabilities` 目录仍保留但不再注册。
- passed：`rule-engine/DashBoard`、`rule-engine/Alarm/Log` 均与目标模块 `baseMenu.json` 的真实菜单 code 对齐；记录回跳会先通过目标模块自有告警查询接口重新校验权限，再打开现有抽屉。
- passed：目标仓库与 `alarm-ui` 相关变更均通过 `git diff --check`；`alarm-ui/register.ts` 的 `moduleId`、`apis` 和两个 `dataCapabilityProviders` 保持完整，现有 API、locale、`dataCapabilities/` 与接口说明脏改动未被覆盖或删除。
- partial：目标模块定向 `vue-tsc` 未发现新增 `agentCapabilities/alarmAnalysis`、`useAlarmAgentEntry.ts` 或 `register.ts` 类型错误；仅命中 `views/Alarm/Log/TabComponent/index.vue` 第 64、375 行两条既有错误，本次在该 560 行历史文件中只新增第 157、517 行的 import 与 hook 调用，未扩大处理无关类型债务。按本机资源约束未执行全量构建。

### 范围扩展：告警数据能力迁移（已执行）

当前目标：将 `alarm-ui` 当前未发布的 `dataCapabilities` 一并迁入 `rule-engine-manager-ui`，由目标模块承接 `alarm:device-monitoring`、`alarm:vision-monitoring` 两个 Provider 和 17 个既有 DataSource ID。此范围扩展覆盖上文“暂不迁移 `alarm-ui/dataCapabilities`、保留源模块 data provider 注册”的旧边界，其余告警智能体迁移结论继续有效。

影响范围与 owning module：

- `ui/modules/rule-engine-manager-ui/dataCapabilities`：迁入设备告警、视联告警 Provider、service 与 types，Provider owner 的 `moduleId` 改为 `rule-engine-manager-ui`。
- `ui/modules/rule-engine-manager-ui/api`：只迁入数据能力实际依赖的告警等级、设备告警查询和聚合请求封装；不复制 `alarm-ui/api/iotAlarm.ts` 中市场升级、空间管理、规则通知等无关逻辑。
- `ui/modules/rule-engine-manager-ui/register.ts`：补充目标 `moduleId` 和两个 `dataCapabilityProviders`，保持 17 个 capability ID 不变。
- `ui/modules/rule-engine-manager-ui/locales/lang/zh.json`、`en.json`：迁入 `AlarmDataCapability.*` 20 个、`VisionAlarmDataCapability.*` 45 个中英文键，并保持 key 集合对称。
- `ui/modules/rule-engine-manager-ui/docs`：迁入并改写数据能力接口说明的 owning module、代码入口和验证记录。
- `ui/modules/alarm-ui`：撤销 8 个未发布 data capability 新文件、1 个新增接口说明及 3 个 API、2 个 locale 中仅服务数据能力的未发布改动；`register.ts` 移除 data provider 注册，只保留已确认的旧智能体取消注册。旧 `agentCapabilities` 目录仍不删除。

明确不做：

- 不修改 17 个 DataSource ID、入参、返回结构、轮询取消信号或后端接口契约。
- 不修改大屏、设备或视联数据能力的消费方，不调整告警页面壳层和交互。
- 不让目标模块 import、动态加载或通过模块 API 间接引用 `alarm-ui`。
- 不复制 `alarm-ui` 中与本次 17 个数据源无关的 marketplace、space、告警规则与通知配置逻辑。
- 默认不执行全量构建；先执行数据能力定向测试、类型过滤和静态边界检查。

实施步骤：

1. 先将当前已暂存的 8 个 data capability 文件和接口说明完整复制到目标仓库，再调整相对 API import、Provider owner 与文档 owning module。
2. 在目标 `api` 下建立最小自包含请求封装；告警等级查询同时作为智能体与数据能力的共享事实源，避免目标模块继续维护两份等级归一化逻辑。
3. 在目标 `register.ts` 注册两个 Provider，迁入中英文 65 个数据能力键，并校验 manifest capability ID 与 Provider 实际 source 完全一致。
4. 目标迁移验证通过后，再从 `alarm-ui` 撤销这批未发布新增和对应 API/locale/register 改动；不删除旧 `agentCapabilities` 目录。
5. 执行 17 个 source 静态契约检查、数据服务定向测试、locale JSON/key 对称检查、目标代码 `alarm-ui` 零引用扫描、相关仓库 `git diff --check` 与定向 `vue-tsc` 过滤。

风险与验证：

- 这 8 个源文件和接口说明当前已经位于 `ui` 仓库暂存区。推荐按“真正迁移”处理：目标验证通过后，从源仓库取消暂存并删除这 9 个未发布文件，同时回退源 API/locale 的数据能力专用改动；这样 `alarm-ui` 不会继续显示这批新增文件，也不会重复注册 Provider。
- `alarm-ui/api/iotAlarm.ts` 还包含 marketplace、space 和告警规则通知等既有页面能力，不能整文件搬运或整文件回退；只迁移并回退本次数据能力使用的请求配置扩展，保留源页面既有功能。
- 真实接口验证仍依赖带登录态、资产权限和告警样本的运行环境；本次先保证编译边界、Provider 契约和既有定向测试，浏览器联调若未执行将在验证结果中明确记录。

验证结果：

- passed：新增 `pnpm run test:data-capabilities`，真实打包两个 Provider 后校验 owner、17 个 capability ID、Provider 实际 sources 与 `register.ts` manifest 完全一致。
- passed：`pnpm run test:alarm-agent`、`pnpm run test:agent-tools` 全部通过；共享告警等级 API 和聚合 API 后，智能体能力未回归失败。
- passed：目标中英文 locale JSON 可解析，分别包含 `AlarmDataCapability.*` 20 个、`VisionAlarmDataCapability.*` 45 个且 key 集合完全对称。
- passed：目标生产代码对 `alarm-ui` 的 import、动态加载和模块 API 引用为零；目标 `api/iotAlarm.ts` 未带入 marketplace、space、告警规则或通知配置等源模块私有依赖。目标仓库与源 `alarm-ui` 变更均通过 `git diff --check`。
- passed：清理期间 `ui` 基线已并发更新到数据能力提交 `d34aaa47`；未改写或回退 Git 历史，而是在最新基线上生成该提交对 `alarm-ui` 的精确逆向工作区差异，再额外保留旧智能体取消注册。源暂存区为空，旧 `alarm-ui/agentCapabilities/alarmAnalysis` 12 个文件仍保留。
- partial：定向 `vue-tsc` 未发现新增 data capability、API、agent capability 或 register 类型错误，仅保留 `views/Alarm/Log/TabComponent/index.vue` 第 64、375 行两条既有错误。按本机资源约束未执行全量构建，也未重复执行需要登录态、资产权限和告警样本的浏览器真实接口联调。
