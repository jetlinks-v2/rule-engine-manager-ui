...
iframe executor 只返回 verified `topology` 快照，不发 `presentation`。父页 adapter 在 `complete-topology` 已满足、快照未截断且至少 2 节点 1 连线、且 iframe 未带 presentation 时，用同一份 topology 合成 `presentation.mermaid`，再发出 `topology-diagram` binding。已有 mermaid 仍按精确匹配校验，错误图不覆盖。`partial-draft`、失败、回滚、单节点或截断快照不合成。模型正文不得再写 Mermaid / ASCII / 拓扑清单，图由客户端从 binding 组合。
...
当前戳为 `2026091621`。父页 `orderRuleEditorRemoteTools` 以 `agentVisible !== false` 再过滤一次模型目录；隐藏工具仍可通过 iframe `executeTool` 内部调用。
