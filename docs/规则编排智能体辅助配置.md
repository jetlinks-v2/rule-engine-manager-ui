...
iframe executor 只返回 verified `topology` 快照，不发 `presentation`。父页 adapter 在 `complete-topology` 已满足、快照未截断且至少 2 节点 1 连线、且 iframe 未带 presentation 时，用同一份 topology 合成 `presentation.mermaid`，再发出 `topology-diagram` binding（`text/vnd.mermaid` + `diagram.flowchart`）。已有 mermaid 仍按精确匹配校验，错误图不覆盖。`partial-draft`、失败、回滚、单节点或截断快照不合成。规则编排模块注册 preferred `flowchart` 展示能力（`deliveryPolicy=preferred`），不改全局 mermaid `explicit`。模型正文不得再写 Mermaid / ASCII / 拓扑清单 / JSON AnswerSpec / 未验证资源 URI，图由客户端从 binding 自动组合。
FLAT 下该工具必须保持模型可见声明（`exposure=auto`），并排在规则编辑器 iframe clientTools 最前；
计划产物绑定只使用 `canvas-changes`。`steps`/`completion` 优先提交结构化数组/对象；发布 apply/propose `_schema` 同时接纳该值的 JSON 字符串（completion 还可为 mode 字符串），由父页 `coerceApplyCanvasPlanArguments` 解析后再进 iframe。compile `CANVAS_PLAN_SCHEMA` 仍要求真实 array/object。不得把 stringify 写成推荐路径。
...
当前戳为 `2026091712`。父页 `orderRuleEditorRemoteTools` 以 `agentVisible !== false` 再过滤一次模型目录；隐藏工具仍可通过 iframe `executeTool` 内部调用。
...
> 当前契约以文首与第十阶段为准：`topology-diagram` 使用 `text/vnd.mermaid` + `diagram.flowchart`，规则编排模块注册 preferred `flowchart`，全局 mermaid 仍为 `explicit`。下文保留当时用 `application/vnd.mermaid` 绑定的交付记录，不再作为现行 mediaType。
