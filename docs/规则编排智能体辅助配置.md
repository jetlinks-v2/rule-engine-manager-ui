...
#### 节点 ID 冲突与 NodeType 导入生命周期优化计划

当前 Node-RED 导入对话框使用 `generateIds=true`，会重写本次导入的全部节点 ID。普通连线可由编辑器核心同步重写，
但节点类型私有配置中保存的节点引用无法获知旧 ID 与新 ID 的对应关系。例如 `zip-output.mapping` 使用直接上游节点
ID 作为 Map key，导入后会保留旧 key，导致聚合运行时无法按新来源节点 ID 找到业务 alias。

目标：

- `generateIds=true` 表示允许编辑器自动解决 ID 冲突，但只给当前画布中已经存在同名 ID 的导入对象生成新 ID；无冲突 ID 原样保留。
- 为 NodeType 增加同步导入生命周期 `onimport(idMap)`；`this` 为导入后的节点，`idMap` 提供本次导入对象从旧 ID 到最终 ID 的完整映射。
- `zip-output` 通过自身 `onimport` 重写 `mapping` 的 key，保留对应 alias，不在编辑器核心硬编码节点类型或业务字段。
- 复制粘贴行为保持可用：被复制节点在当前画布中均已存在，因此仍会自动生成新 ID。

影响范围与 owning module：

- `/Users/zhouhao/IdeaProjects/rule-engine-editor/packages/node_modules/@node-red/editor-client/src/js/nodes.js`：逐对象检测 ID 冲突、维护 ID 映射，并在标准引用重写完成后、节点校验前调用 NodeType 的 `onimport`。
- `/Users/zhouhao/IdeaProjects/rule-engine-editor/packages/node_modules/@node-red/editor-client/public/red/red.js` / `red.min.js`：由编辑器源码构建生成，不直接手改生成逻辑。
- `modules/rule-engine-manager/src/main/resources/static/rule-editor/red/red.js` / `red.min.js`：同步经过验证的编辑器构建产物。
- `modules/rule-engine-manager/src/main/resources/rule-engine/editor/common/zip-output.html`：实现 `onimport(idMap)`，只重写能够在本次导入映射中解析到的来源节点 key。

不做：

- 不关闭 ID 冲突保护，不让同一画布出现重复 ID。
- 不按节点名称、坐标或连线顺序猜测新旧节点关系。
- 不递归扫描并替换节点全部 JSON 配置，避免误改脚本、模板和业务数据中的普通字符串。
- 不在运行时后端或父页面重复实现 Node-RED 导入逻辑，也不自动迁移已经保存且丢失新旧 ID 关系的历史错误配置。

实施步骤：

1. 将导入器的“全部生成新 ID”判断拆为按原始 ID 的冲突集合；分别处理对象自身 ID 重写和 workspace / subflow 容器引用重写，避免只改部分节点时破坏 `z`、分组和连线。
2. 在导入上下文中构造包含 workspace、subflow、配置节点、普通节点、group 和 junction 的旧 ID 到最终 ID 映射；无冲突项映射到自身。
3. 仅在交互式导入 / 复制链路中，对新导入且已注册的 NodeType 同步调用一次 `onimport(idMap)`；普通初始加载、历史撤销恢复和 reimport 不触发该生命周期。回调异常按现有 NodeType 生命周期风格隔离并输出明确类型信息，避免留下半完成导入事务。
4. 在 `zip-output` 的导入回调中规范化 mapping，按 `idMap[oldSourceId]` 重建 key 并原样保留 alias；未包含在本次导入图中的来源引用不做猜测性匹配。
5. 构建 `rule-engine-editor`，检查生成差异后同步 `red.js` / `red.min.js` 到 `rule-engine-manager`，再完成静态和浏览器回归。

风险与待确认点：

- 这是已存在 `generateIds` 语义的行为收敛；调用方仍可通过显式 `importMap[id]="copy"` 强制单个对象生成新 ID，`replace` 等既有冲突策略保持不变。
- 部分冲突导入要求容器 ID 和子节点 ID 分开判断：workspace / subflow 改 ID 后，即使子节点自身不冲突，也必须重写其 `z` 和相关类型引用。
- `onimport` 是公共 NodeType 契约，必须在代码旁说明调用时机、同步修改边界和异常处理；不支持异步返回值或在回调中新增 / 删除节点。
- 已经错误保存的 `zip-output.mapping` 缺少可靠的新旧 ID 关系，只能重新导入原 JSON 或人工校正，不能按名称或顺序自动修复。

验证方式：

- 导入完全无冲突的规则：所有节点 ID 保持不变，连线、workspace / subflow 和配置节点引用正确。
- 导入部分 ID 冲突的规则：只重写冲突对象，所有指向它的标准引用与 NodeType 私有引用同步更新。
- 复制粘贴已有节点：复制对象全部获得新 ID，原有复制语义不回退。
- 导入包含 `zip-output` 的双分支规则：mapping key 等于导入后的直接上游节点 ID，alias 保持不变；运行时聚合字段不退化成随机节点 ID。
- 验证无冲突、部分冲突、显式 `importMap=copy/replace`、subflow、group / junction、撤销恢复和编辑器初始加载；执行 `npm test` 或收敛后的相关测试、`npm run build`、`node --check`、`git diff --check`，并在真实规则编辑器完成导入后保存回归。

实施结果与验证：

- 已在 `rule-engine-editor` 源码的 `nodes.js` 实现冲突集合、完整 `idMap`、容器引用重写和 `onimport(idMap)` 生命周期；`generateIds=true` 下无冲突 ID 保持不变，复制链路仍会为已有 ID 生成新 ID。
- `zip-output` 已通过 `onimport` 重建 `mapping` key 并保留 alias；无法从本次导入映射解析的 key 原样保留。构建后的 `red.js` / `red.min.js` 已同步到 `rule-engine-manager` 静态资源目录。
- 通过：`npm run build`、`node --check`（源码与构建产物）、zip-output 内嵌脚本语法检查、`git diff --check`、Maven `resources:resources`，以及无冲突 / 部分冲突 / 重复导入 / subflow 容器的导入行为回归脚本。
- `npm test` 的 JSHint 阶段仍报告源仓库既有的 5 个非本次改动告警；全量 `simplemocha:all` 受本机文件系统测试权限 / 环境依赖影响出现既有失败，未将这些环境失败归因于本次导入改动。真实浏览器编辑器回归仍需在可用的规则编辑器实例中执行。
