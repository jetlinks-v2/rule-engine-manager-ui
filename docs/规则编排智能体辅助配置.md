...
### 规则实例列表表格操作列标准化（2026-09-23）

- 目标：表格形态的操作列改用 `TableActions` / `TableActionsItem`，统一行内常用操作与更多操作的承载方式。
- 影响范围与 owning module：仅调整 `runtime-ui/modules/rule-engine-manager-ui/views/Instance/index.vue` 的表格操作列；`runtime-ui/modules/rule-engine-manager-ui` 为 owning module。
- 不做：不修改卡片形态、操作权限码、启停/删除确认逻辑、接口与数据结构。
- 实施步骤：保留“编排、编辑”为行内常用操作，将“启用/禁用、删除”放入更多面板，并在面板中展示图标与文案。
- 风险与待确认点：需要确认更多面板中的权限禁用、二次确认以及表格行事件隔离仍沿用现有组件契约。
- 验证方式：执行模块构建并在规则编排表格形态下检查行内操作、更多面板、启停确认和删除禁用态。
- passed：`pnpm -C runtime-ui build:modules rule-engine-manager-ui` 构建通过；仅输出既有的 Baseline 数据过期、CSS `//` 注释、Rollup output option 和大 chunk 警告。
- checked：`pnpm -C runtime-ui exec vue-tsc -p modules/rule-engine-manager-ui/tsconfig.json --noEmit --pretty false` 仍被 core、SceneLinkage、可视化等既有类型错误阻断；对输出定向检索后未发现 `views/Instance/index.vue` 类型错误。当前 workspace 未提供 lint 脚本，使用 `git diff --check` 完成变更格式检查并通过。
- checked：在 `http://localhost:9200/#/rule-engine/Instance` 切换到表格形态，确认每行只外置“编排、编辑”，更多面板展示“启用/禁用、删除”；正常状态规则的删除按钮保持禁用，启用操作仍会弹出“确认启用?”，验证时已取消确认，未改变业务数据。
- commit：`183e9c9`（`refactor(规则引擎): 统一列表操作与表格布局`）
- PR：<https://github.com/jetlinks-v2/rule-engine-manager-ui/pull/122>
